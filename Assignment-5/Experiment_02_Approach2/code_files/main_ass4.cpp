
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <omp.h>
#include "init.h"
#include "utils.h"

int GRID_X, GRID_Y, NX, NY;
int NUM_Points, Maxiter;
double dx, dy;

int main(void) {
    /* approach: no_delete */
    int grids[][2] = { {250,100}, {500,200}, {1000,400} };
    int ngrids = 3;
    int thread_counts[] = { 1, 2, 4, 8, 16 };
    int ntc = 5;
    Maxiter = 10;
    NUM_Points = 14000000;

    FILE *fp = fopen("exp02_parallel_no_delete.csv", "w");
    if (!fp) { printf("Can't open file\n"); return 1; }
    fprintf(fp, "Grid_NX,Grid_NY,Num_Threads,Total_Interp_Time_s,Total_Mover_Time_s,Total_Time_s,Mover_Speedup\n");

    printf("EXP 2 NO DELETE\n");
    printf("Grid    Thr Interp Mover Total Spd\n");

    for (int gi = 0; gi < ngrids; gi++) {
        NX = grids[gi][0];
        NY = grids[gi][1];
        GRID_X = NX + 1;
        GRID_Y = NY + 1;
        dx = 1.0 / NX;
        dy = 1.0 / NY;
        const char *gname = (NX == 250 ? "250x100" : NX == 500 ? "500x200" : "1000x400");

        double serial_mover_time = 0.0;

        for (int ti = 0; ti < ntc; ti++) {
            int nt = thread_counts[ti];

            double *mesh_value = (double *)calloc((long)GRID_X * GRID_Y, sizeof(double));
            Points *points = (Points *)calloc((long)NUM_Points, sizeof(Points));
            if (!mesh_value || !points) {
                printf("Alloc failed for grid %dx%d\n", NX, NY);
                fprintf(fp, "%d,%d,%d,ALLOC_FAIL,ALLOC_FAIL,ALLOC_FAIL,0.0\n", NX, NY, nt);
                if (mesh_value) free(mesh_value);
                if (points) free(points);
                continue;
            }

            srand(12345);
            initializepoints(points);

            double total_interp = 0.0;
            double total_mover = 0.0;

            for (int iter = 0; iter < Maxiter; iter++) {
                memset(mesh_value, 0, (long)GRID_X * GRID_Y * sizeof(double));

                clock_t ci0 = clock();
                interpolation(mesh_value, points);
                clock_t ci1 = clock();
                total_interp += (double)(ci1 - ci0) / CLOCKS_PER_SEC;

                double wt0 = omp_get_wtime();
                
                if (nt == 1) {
                    /* Serial no-delete: call parallel with 1 thread */
                    mover_parallel_no_delete(points, dx, dy, 1);
                } else {
                    mover_parallel_no_delete(points, dx, dy, nt);
                }

                double wt1 = omp_get_wtime();
                total_mover += (wt1 - wt0);
            }

            if (nt == 1) {
                serial_mover_time = total_mover;
            }

            double speedup = (serial_mover_time > 0.0) ? serial_mover_time / total_mover : 1.0;

            printf("%s %d %.6f %.6f %.6f %.4f\n", gname, nt,
                   total_interp, total_mover, total_interp + total_mover, speedup);
            fprintf(fp, "%d,%d,%d,%.6f,%.6f,%.6f,%.4f\n", NX, NY, nt,
                    total_interp, total_mover, total_interp + total_mover, speedup);

            free(mesh_value);
            free(points);
        }
    }
    fclose(fp);
    printf("Done. See exp02_parallel_no_delete.csv\n");
    return 0;
}
